"""LM Studio native-chat provider."""
...
class LMStudioProvider:
    def __init__(self, *, base_url: str, model: str, temperature: float=0.1,
                 max_output_tokens: int=16384, timeout: float=300.0, http_retries: int=2):
    ...
    def _request(self, *, input_text: str, system_prompt: str) -> str:
        payload={"model":self.model,"input":input_text,"system_prompt":system_prompt,
                 "temperature":self.temperature,"max_output_tokens":self.max_output_tokens,"store":False}
        ...
        for attempt in range(self.http_retries+1):
            try:
                ...
                last_exc=exc
                if attempt>=self.http_retries: raise
    ...
    def extract(self, envelope: HarvestInputEnvelope) -> dict[str, Any]:
        text=self._request(input_text=json.dumps(envelope.model_dump(mode="json"),ensure_ascii=False),
                           system_prompt=EXEGATE_HARVEST_SYSTEM_PROMPT)
        try:
            return parse_json_object(text)
        except ModelResponseError:
            repaired=self._request(
                input_text=json.dumps({"source_envelope":envelope.model_dump(mode="json"),"invalid_response":text},ensure_ascii=False),
    ...
    def repair(self, candidate: dict[str, Any], validation_errors: str, envelope: HarvestInputEnvelope) -> dict[str, Any]:
        text=self._request(
            input_text=json.dumps({"source_envelope":envelope.model_dump(mode="json"),"invalid_candidate":candidate,
                                   "validation_errors":validation_errors},ensure_ascii=False),
